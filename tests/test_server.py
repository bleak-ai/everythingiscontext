...
def test_list_resources_applies_display_names(project, monkeypatch):
    """Modules and connections with a names: override in controls.yaml show
    the custom display name; those without keep the default."""
    import asyncio

    from gcontext import commands as commands_mod

    # Two modules: m_custom gets a display override, m_plain does not.
    (project / "modules" / "m_custom").mkdir(parents=True)
    (project / "modules" / "m_custom" / "index.md").write_text("x")
    (project / "modules" / "m_plain").mkdir(parents=True)
    (project / "modules" / "m_plain" / "index.md").write_text("x")

    # One connection with override.
    (project / "connections" / "c_custom").mkdir(parents=True)
    (project / "connections" / "c_custom" / "connection.yaml").write_text(
        "name: c_custom\ndescription: d\nkind: product-api\n"
    )

    # controls.yaml with names: overrides.
    (project / "controls.yaml").write_text(
        "names:\n"
        "  modules/m_custom: My module\n"
        "  connections/c_custom: My API\n"
    )
    ...
    try:
        resources = asyncio.run(tracker.on_list_resources(None, call_next))
    finally:
        commands_mod._REGISTRY, commands_mod._ROOT = saved_registry, saved_root

    by_uri = {str(r.uri): r for r in resources}

    # Module with override shows custom name.
    m_custom_uri = [u for u in by_uri if "modules/m_custom" in u]
    assert len(m_custom_uri) == 1
    assert by_uri[m_custom_uri[0]].name == "My module"

    # Module without override keeps default.
    m_plain_uri = [u for u in by_uri if "modules/m_plain" in u]
    assert len(m_plain_uri) == 1
    assert by_uri[m_plain_uri[0]].name == "modules / m_plain"

    # Connection with override shows custom name.
    c_custom_uri = [u for u in by_uri if "connections/c_custom" in u]
    assert len(c_custom_uri) == 1
    assert by_uri[c_custom_uri[0]].name == "My API"

    # Group headers keep their plain names.
    modules_header = [r for r in resources if r.name == "modules"]
    assert len(modules_header) == 1
    connections_header = [r for r in resources if r.name == "connections"]
    assert len(connections_header) == 1
